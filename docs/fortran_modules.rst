======================
FORTRAN Modules Source
======================

Below are the headers from the FORTRAN modules of the decades-pp library. The source code can be found in the repository on `github <https://github.com/ncasuk/decades-pp/tree/master/ppodd/pod/fortran_modules>`_.

------------
c_airspd.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_airspd.for
   :language: fortran
   :end-before: C*************
  
-----------
c_check.for
-----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_check.for
   :language: fortran
   :end-before: *************
   
----------   
c_comr.for
----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_comr.for
   :language: fortran
   :end-before: C*************

------------
c_geneas.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_geneas.for
   :language: fortran
   :start-after: C----------------
   :end-before: C----------------

------------
c_grflux.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_grflux.for
   :language: fortran
   :start-after: C----------------
   :end-before: C----------------

----------
c_gsun.for
----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_gsun.for
   :language: fortran
   :end-before: C#################################

------------   
c_gwinds.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_gwinds.for
   :language: fortran
   :end-before: *************
   
------------
c_heiman.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_heiman.for
   :language: fortran

----------
c_ins1.for
----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_ins1.for
   :language: fortran
   :end-before: C*************
   
---------
c_lwc.for
---------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_lwc.for
   :language: fortran
   :end-before: C####################

------------   
c_nephl1.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_nephl1.for
   :language: fortran
   :start-after: !!!!!!!!!!!!!!!!!!!!!
   :end-before: !!!!!!!!!!!!!!!!!!!!!

----------   
c_nevz.for
----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_nevz.for
   :language: fortran
   :end-before: C*************

---------   
c_nox.for
---------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_nox.for
   :language: fortran
   :end-before: C*************

------------   
c_ozone1.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_ozone1.for
   :language: fortran
   :end-before: C*************
   
------------
c_press1.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_press1.for
   :language: fortran
   :end-before: !*************

-----------   
c_press.for
-----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_press.for
   :language: fortran
   :end-before: C*****************

----------   
c_psap.for
----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_psap.for
   :language: fortran
   :start-after: !!!!!!!!!!!!!!!!!!!!!!!!
   :end-before: !!!!!!!!!!!!!!!!!!!!!!!!

------------   
c_radal1.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_radal1.for
   :language: fortran
   :end-before: !********************

-----------   
c_rflux.for
-----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_rflux.for
   :language: fortran
   :start-after: C---------------------
   :end-before: C---------------------

----------      
c_rvsm.for                                                                                                                                                                 
----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_rvsm.for
   :language: fortran
   :end-before: !*******************

---------   
c_so2.for                                                                                                                                                                  
---------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_so2.for
   :language: fortran
   :end-before: C*************

----------   
c_sols.for                                                                                                                                                                 
----------   

.. literalinclude:: ../ppodd/pod/fortran_modules/c_sols.for
   :language: fortran
   :start-after: C----------------
   :end-before: C----------------

---------   
c_sun.for                                                                                                                                                                  
---------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_sun.for
   :language: fortran
   :end-before: C####################

------------   
c_temps2.for
------------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_temps2.for
   :language: fortran
   :end-before: C*************

------------   
c_tpress.for
------------   

.. literalinclude:: ../ppodd/pod/fortran_modules/c_tpress.for
   :language: fortran
   :end-before: !*******************

----------   
c_turb.for
----------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_turb.for
   :language: fortran
   :start-after: !!!!!!!!!!!!!!!!!!!!!!!!!
   :end-before: !!!!!!!!!!!!!!!!!!!!!!!!!

---------   
c_twc.for
---------

.. literalinclude:: ../ppodd/pod/fortran_modules/c_twc.for
   :language: fortran
   :end-before: C#################

-----------     
c_winds.for
-----------   

.. literalinclude:: ../ppodd/pod/fortran_modules/c_winds.for
   :language: fortran
   :end-before: *************
   
----------   
g_mach.for
----------

.. literalinclude:: ../ppodd/pod/fortran_modules/g_mach.for
   :language: fortran
   :start-after: C------------------------
   :end-before: C------------------------

-----------     
isetflg.for
-----------   

.. literalinclude:: ../ppodd/pod/fortran_modules/isetflg.for
   :language: fortran
   :end-before: C*************

-----------   
itstflg.for
-----------   

.. literalinclude:: ../ppodd/pod/fortran_modules/itstflg.for
   :language: fortran
   :end-before: C*************

----------   
s_mach.for
----------   

.. literalinclude:: ../ppodd/pod/fortran_modules/s_mach.for
   :language: fortran
   :start-after: C------------------------
   :end-before: C------------------------


----------   
s_qcpt.for
----------   

.. literalinclude:: ../ppodd/pod/fortran_modules/s_qcpt.for
   :language: fortran
   :start-after: C------------------------
   :end-before: C------------------------

---------   
s_sun.for
---------

.. literalinclude:: ../ppodd/pod/fortran_modules/s_sun.for
   :language: fortran
   :end-before: C##################
