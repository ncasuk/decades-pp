...
def get_no_cloud_mask(twc_col_p, wow):
    ...
    mask[(rng > rng_limits[0]) &
         (rng < rng_limits[1]) & (np.max(wow, axis=1) == 0)] = 1
...
class rio_nevzorov_1t2l(cal_base):
    """
    """
    def __init__(self,dataset):
        ...
        if not dataset.has_key('VANETYPE'):
            dataset['VANETYPE'] = [1.0,]

        if int(dataset['VANETYPE'][0]) == 2:
            self.input_names.remove('CALNVLWC')
        else:
            self.input_names.remove('CALNVLWC1')
            self.input_names.remove('CALNVLWC2')
