...
def calc_uncertainty(buck_mirr_temp, buck_pressure, buck_mirr_ctrl_flag):
    ...
    for i in range(0, n):
        ...
        if ((i > lag) & (i < (n-lag))):
            fwdUt=np.stddev(buck_mirr_temp[i:i+lag])
            backUt=np.stddev(buck_mirr_temp[i-lag:i])
        else:
            fwdUt= 0
            backUt=0      
    
    
        if (buck_pressure[i] > 0 ):
...
##;;input it buck_data structure
def get_buck_mirror_ctl(buck_mirr_temp):
    """routine that uses temperature change (DT) to devise flagging for Mirror Temperature measurement based on whether
    ...
    for i in range(interval+1, buck_mirr_temp.size-interval-1):
        ...
        culm2=0
        e=0
        #;;//do an average of [interval] data to determine the rate of change of Mirror Temp
        while e < interval:
        ...
        #;;//Make a first cut at guessing the mirror state - 0=water (above 273K), 
        #;;//1=ice (when the mirror has been cold and then not above zero) and 2=not known.
...
def get_enhance_coeff(buck_mirror_ctl):
    """
    """
    #rows=n_elements(buck_mirror_ctl)
    ...
    ix0=np.where(buck_mirror_ctl < 1)[0]
    ix1=np.where((buck_mirror_ctl > 1) & (buck_mirror_ctl != 3))[0]
    ix2=np.where(buck_mirror_ctl == 3)[0]

    result[ix0,:]=ice_coeff
    result[ix1,:]=wat_0to100_coeff
    result[ix2,:]=np.nan
    return result
...
def get_vp_coeff(buck_mirror_ctl):
    ...
    ix_wat=np.where(buck_mirror_ctl < 1)[0]
    ix_ice=np.where(buck_mirror_ctl > 1)[0]
    ix_nan=np.where(buck_mirror_ctl > 2)[0]

    result[ix_wat,:]=wat_coeff
    result[ix_ice,:]=ice_coeff
    result[ix_nan,:]=np.nan
    return result



def calc_vp(buck_mirror_temp, buck_mirror_ctl, buck_unc_k=None):

    # Go through the data and calculate vapour pressure depending on
    # likely mirror state (ie water above 273.15K, Ice below that)

    
    if not hasattr(buck_unc_k, 'size'):
        n=buck_mirror_temp.size
        buck_unc_k=np.zeros(n, dtype=np.float32)

    c=get_vp_coeff(buck_mirror_ctl)

    result=np.exp(c[:,0]/(buck_mirror_temp+buck_unc_k)**2 + \
                  c[:,1]/(buck_mirror_temp+buck_unc_k) + \
                  c[:,2] + \
                  c[:,3]*(buck_mirror_temp+buck_unc_k) + \
                  c[:,4]*(buck_mirror_temp+buck_unc_k)**2 + \
                  c[:,5]*(buck_mirror_temp+buck_unc_k)**3 + \
                  c[:,6]*(buck_mirror_temp+buck_unc_k)**4 + \
                  c[:,7]*(np.log( buck_mirror_temp+buck_unc_k))) 
    return result


def calc_vmr(vp, enhance, buck_pressure):
    vmr=vp/(buck_pressure*100.0-vp*enhance)*enhance
...
def calc_enhance_factor(vp_buck, buck_mirror_t, buck_pressure, buck_mirror_ctl):
    ...
    result=np.exp((1.0-vp_buck/(buck_pressure*100.0))*\
                  c[:,0]+c[:,1]*buck_mirror_t+c[:,2]*buck_mirror_t**2+c[:,3]*buck_mirror_t**3) + \
...
class rio_buck_cr2(cal_base):                                                                                                                                                          
    ...
    def process(self):
        ...
        buck_mirr_pressure=self.dataset['AERACK_buck_pressure'].ismatch(match)
        buck_mirr_ctrl_flag=self.dataset['AERACK_buck_mirr_ctrl_flag'].ismatch(match)
        buck_mirr_flag=self.dataset['AERACK_buck_mirr_flag'].ismatch(match)        
        buck_status=self.dataset['AERACK_buck_status'].ismatch(match)        
        ...
        enhance=calc_enhance_factor(vp_buck, self.dataset['AERACK_buck_mirr_temp'], self.dataset['AERACK_buck_pressure'], self.dataset['AERACK_buck_mirr_ctrl_flag'])
        vmr_buck=calc_vmr(vp_buck, enhance, self.dataset['AERACK_buck_pressure'])
        vmr_max=calc_vmr(vp_max, enhance, self.dataset['AERACK_buck_pressure'])
